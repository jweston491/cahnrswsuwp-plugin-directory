...
function _manually_load_plugin() {
	require dirname( dirname( __FILE__ ) ) . '/cahnrswsuwp-plugin-directory.php';
	foreach (scandir( dirname( dirname( __FILE__ ) ) . '/classes'  ) as $filename) {
		$path = dirname( dirname( __FILE__ ) ) . '/classes/' . $filename;
		if (is_file($path)) {
			require_once $path;
		}
	}
